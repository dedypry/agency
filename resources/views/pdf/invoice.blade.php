...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .page { padding: 34px; }
        .header, .bill-row, .totals-row, .footer-row {
            display: table;
            width: 100%;
        }
        ...
        .invoice-title { font-size: 28px; font-weight: 700; letter-spacing: 1px; margin: 0; }
        .badge {
...
<body>
    ...
    <div class="page">
        ...
        <div class="bill-row section">
            <div class="col">
                <div class="muted tiny"><strong>DITAGIHKAN KEPADA</strong></div>
                <div><strong>{{ $invoice->customer_name }}</strong></div>
                @if ($invoice->customer_address)
                    <div class="muted">{{ $invoice->customer_address }}</div>
                @endif
                <div class="muted">{{ collect([$invoice->customer_phone, $invoice->customer_email])->filter()->join(' · ') }}</div>
            </div>
            <div class="col right">
                ...
                @endif
                @if ($invoice->agent)
                    <div><span class="muted">Sales:</span> {{ $invoice->agent->name }}</div>
                @endif
            </div>
        </div>
            ...
            <tbody>
                @foreach ($invoice->items as $item)
                    <tr>
                        ...
                        <td class="number">{{ rtrim(rtrim(number_format((float) $item->quantity, 2, ',', '.'), '0'), ',') }}</td>
                        <td class="number">Rp {{ number_format((float) $item->unit_price, 0, ',', '.') }}</td>
                        <td class="number">Rp {{ number_format((float) $item->total, 0, ',', '.') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="totals">
            <div><span class="muted">Subtotal</span><span class="right">Rp {{ number_format((float) $invoice->subtotal, 0, ',', '.') }}</span></div>
            <div><span class="muted">Diskon</span><span class="right">- Rp {{ number_format((float) $invoice->discount, 0, ',', '.') }}</span></div>
            <div><span class="muted">Pajak ({{ rtrim(rtrim(number_format((float) $invoice->tax_percent, 2, ',', '.'), '0'), ',') }}%)</span><span class="right">Rp {{ number_format((float) $invoice->tax_amount, 0, ',', '.') }}</span></div>
            <div class="grand-total"><span>Total</span><span class="right">Rp {{ number_format((float) $invoice->total, 0, ',', '.') }}</span></div>
        </div>

        <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;font-style:italic;margin-top:22px;padding:10px 12px;">
